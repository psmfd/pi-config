...
The registered `omlx/coding-workhorse` model (GLM-4.7-Flash-6bit alias, cost-0, `contextWindow` held to 131072 for concurrency — local-llm ADR-009/010) flows into the canonical availability snapshot and matrix candidate pool like any built-in. Fourteen first-party read-only wrappers declare `local-llm: true`; this is eligibility, not an exact pin. The set includes `gitflow-expert` after ADR-0123 replaced its `bash` surface with typed read-only Git/GitHub tools and ADR-0149 removed the obsolete mutation-risk exclusion. When the global local role permits it, ADR-0090/ADR-0094 matrix policy can choose the local-first lane after capability filtering. The review trio instead requests `capability-tier: frontier`, and local-forbidden (`tools` omitted or `bash`-tool) wrappers remove local candidates and fail closed without a non-local pick. No first-party wrapper currently carries `model:`; explicit third-party pins remain authoritative and use the ADR-0080/ADR-0081 spawn-time fallback/liveness gate. Known accepted limitation: oMLX overflow errors are not normalized to `context_length_exceeded`, so auto-compaction may not fire on overflow (#518).
