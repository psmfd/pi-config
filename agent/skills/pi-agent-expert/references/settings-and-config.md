...
The registered `omlx/coding-workhorse` model (GLM-4.7-Flash-6bit alias, cost-0, `contextWindow` held to 131072 for concurrency — local-llm ADR-009/010) flows into `shared/candidates.getCandidates()` and the auto-router menu like any built-in. Thirteen read-only specialist wrappers (those carrying no `bash` tool) pin it via frontmatter, behind the subagent extension's spawn-time registry gate (ADR-0076, refined by the `has-bash → primary` discriminator in ADR-0085) — hosts without this operator-local block run those subagents on the Copilot fallback rung (`github-copilot/gpt-5-mini` by default, #536/ADR-0080) or, failing that, the session default. Known accepted limitation: oMLX overflow errors are not normalized to `context_length_exceeded`, so auto-compaction may not fire on overflow (#518).
