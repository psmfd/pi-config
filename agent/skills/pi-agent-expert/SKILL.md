...
| Our vendored `subagent` extension — current discovery/frontmatter, model/env/expertise policy, concurrency, event stream, patch-manifest boundary, spawn shape | [`references/subagent-internals.md`](references/subagent-internals.md) |
...
- **Doc-vs-runtime event-name drift.** `docs/json.md` and `docs/extensions.md` are usually authoritative, but event fields shift. Pi v0.80.6 emits `tool_execution_start`/`tool_execution_update`/`tool_execution_end`; upstream's example still carries a dead `tool_result_end` branch, while local patch #3 consumes all three current edges. Re-verify against the installed runtime on every vendor audit.
...
- **Frontmatter is whitelisted, not free-form.** Our `agent/extensions/subagent/agents.ts` reads `name`, `description`, `tools`, `model`, `guard-profile`, `env-strict`, `env-allow`, `env-allow-prefix`, `local-llm`, and `capability-tier`. Adding other keys (for example `thinking` or `max_turns`) requires extending the parser and the corresponding policy/argv/environment seam. Pi's skill-loader whitelist is separate.
...
- **`disable-model-invocation: true`** hides a skill from `<available_skills>` (no auto-trigger). Users invoke it via `/skill:name`, or the matching agent wrapper loads it explicitly via `read` inside the spawned subprocess. All 21 wrapper-paired skills in this repo set this flag — it forces routing through the `subagent` tool and trims the parent system prompt. The three review specialists are *additionally* gated by `capability-tier: frontier`, specialist tool allowlists, and explicit read-only contracts; the matrix chooses the concrete model. See `agent/AGENTS.md`.
