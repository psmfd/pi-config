...
import { assertMutationAllowed } from "./containment";
...
async function executeEditPipeline(
	...
	if (primaryError !== null) {
		...
		if (!isStale || !absolutePath) {
			throw primaryError instanceof Error ? primaryError : new Error(JSON.stringify(primaryError));
		}
...
function buildEditToolDefinition(): EditToolDefinition {
	...
	async execute(_toolCallId, params, signal, _onUpdate, ctx) {
		...
		assertMutationAllowed(mutationTargetPath, ctx.cwd, path); // pi_config patch #5 — see PATCH_MANIFEST.json
