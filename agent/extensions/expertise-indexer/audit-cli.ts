...
import { computeCanonicalBlob, isValidGitSha } from "./canonicalize.ts";
...
async function main(): Promise<number> {
	...
	const origin = defaultGit(["config", "--get", "remote.origin.url"]);
			...
			JSON.stringify(
				{
					...
					search: search.ok
						? {
								...
								// Scan once, reuse — redact to category names on a hit (#817).
								body: (() => {
									const hits = scanRawString(search.text);
									return hits.length > 0 ? `[redacted:${hits.join(",")}]` : search.text;
								})(),
							}
