...
| 9 | Subagent policy model seam | `index.ts` (`selectSubagentPolicyModel`, per-tool `getCandidates` + `loadRoutingMatrix`, and `runSingleAgent` policy-model threading) | Parent subagent invocation now has a deterministic policy seam before spawn: explicit wrapper `model:` pins remain authoritative; unpinned local-forbidden agents (`tools` omitted, or `tools` includes `bash`) ask the shared capability matrix for a non-local provider model using local-filtered cheapest-capable ranking; unpinned local-eligible agents default to `omlx/coding-workhorse` when available. A resolved policy model is still passed through the existing spawn-time pin gate, so unavailable ids fall back safely and child auto-router processes stay inert when a concrete `--model` is emitted. If a local-forbidden agent has no non-local matrix pick, the child fails closed rather than inheriting a possibly-local session model. | pi_config #656, pi_config #657, [ADR-0090](../../../adrs/0090-stable-router-subagent-model-policy.md) |
...
(ADR-0090): local-forbidden agents — wrappers that omit `tools` (pi defaults) or
