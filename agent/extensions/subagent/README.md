...
| 17 | Bounded runtime provider failover | `index.ts` (structured child-error/tool-edge tracking, one-attempt reselection, usage + failover telemetry, result rendering, shared session reset), `policy-model.ts` (session-deny filtering), `test/runtime-failover.test.ts`, `test/policy-model.test.ts`; shared state/classifier in `../shared/session-unavailable.ts` | A policy-selected child that receives a structured 429/quota error before any tool event marks its exact `provider/id` unavailable and retries once on the next eligible matrix model from the same ADR-0104 snapshot. Explicit pins, session-default children, stderr/generic failures, aborts, and any post-tool failure never replay. Parallel tasks share dynamic deny evidence; telemetry names attempted/fallback models and snapshot identity without copying raw provider errors. Since [ADR-0126](../../../adrs/0126-provider-session-circuit-breaker.md) the shared state carries a second, `provider` scope: an operator `/auto providers disable <p>` — or two distinct rate-limited models of one provider — removes every row of that provider from unpinned child selection, including rows nothing probed. Explicit pins are not yet gated by the breaker (#903). | pi_config #868, [ADR-0122](../../../adrs/0122-subagent-runtime-provider-failover.md), [ADR-0126](../../../adrs/0126-provider-session-circuit-breaker.md) |
...
| 19 | Provider-breaker fail-closed for pins and the Copilot rung | `index.ts` (`COPILOT_PROVIDER`, `failoverExhausted`, provider-deny gate between `applyLocalRole` and `resolveModelPin`, `disabledReason` on the rung, `provider-breaker` failover outcome + rendering), `model-pin.ts` (`CopilotFallback.disabledReason`, rung skip), `test/runtime-failover.test.ts`, `test/model-pin.test.ts` | [ADR-0126](../../../adrs/0126-provider-session-circuit-breaker.md) gave the shared deny state a `provider` scope, but two paths still bypassed it. (1) An explicit wrapper `model:` pin naming a broken provider would spawn a child onto a provider already known dead this session; it now fails closed with `stopReason: "policy-blocked"`, naming the breaker's source, reason, and timestamp plus the `/auto providers enable <p>` remedy. Deliberately **provider scope only** — ADR-0122's "explicit pins remain authoritative and return their own failure" continues to hold for model-scope denies. Evaluated after `applyLocalRole`, so a pin the lever already dropped is never reported as breaker-refused. (2) The ADR-0080 Copilot rung would substitute `github-copilot/<id>` for a dropped non-Copilot pin even with the Copilot breaker tripped; the rung is now carried through as disabled (not dropped, which would be indistinguishable from missing registry data) so the note explains why. Reselection exhausted by a breaker reports `provider-breaker` rather than a bare `no-alternate` — same terminal state, different operator action. The rung's breaker state is stamped per spawn (`liveCopilotRung`), not at fan-out setup: `buildCopilotFallback` runs once per tool call, so a breaker tripped by child 1 must still reach children 2..N. | pi_config #903, [ADR-0126](../../../adrs/0126-provider-session-circuit-breaker.md) |
...
sequenceDiagram
    ...
    Model->>Pi: call tool subagent (single | parallel | chain)
...
```text
Run gh-cli-expert and gitflow-expert in parallel: one on the release commands, one on the branch flow
```
...
flowchart TD
    ...
    InheritProfile --> Dispatch["dispatch: single / parallel / chain"]
...
`scripts/test-subagent.sh` runs the 9 `test/*.test.ts` suites (node:test via
...
- Output truncated to last 10 items in collapsed view (expand to see all);
  parallel-mode per-task text summaries are additionally capped at 50 KB
