...
| 13 | Capability-tier quality floor | `agents.ts` (`capabilityTier` on `AgentConfig`, parsed from `capability-tier` frontmatter — exact enum values only, typos yield untiered selection), `policy-model.ts` (tier branch via `shared/model-ranking.ts` `resolveTierPick`), `test/policy-model.test.ts` | #656: a wrapper declaring `capability-tier: frontier\|capable\|fast` asks the provider matrix for the highest-quality credentialed model at or above that tier — quality-first ordering (tier rank desc, window desc, lexical; cost drops out), or-better semantics, falling through to untiered cheapest-capable when no tiered row qualifies, still respecting the ADR-0094 local-eligibility pool. Replaced the review trio's exact `github-copilot/claude-opus-4.7` pins; no first-party wrapper carries a `model:` pin anymore (pinned by test; the pin remains a documented escape hatch for evals/bad-version dodges). Provider-agnostic: matrix rows for any provider qualify identically; uncredentialed providers' rows are inert. | pi_config #656, [ADR-0094](../../../adrs/0094-local-llm-role-lever.md), [ADR-0090](../../../adrs/0090-stable-router-subagent-model-policy.md) |
...
- **Model at invocation**: The resolved child model is shown from the moment of spawn (local patch #7, pi_config #643) — a pinned child (e.g. `omlx/coding-workhorse`) is visible immediately; an unpinned child fills in once its first turn lands
...
- **Default** (no frontmatter keys): full passthrough minus the always-deny list (`PI_EXPERTISE_ALLOW_LOCALDEV_WRITE`). Retained as the safety net for third-party drop-in wrappers.
...
Before the pin ladder, unpinned agents can receive a policy-selected model
(ADR-0091): local-forbidden agents — wrappers that omit `tools` (pi defaults) or
explicitly include `bash` — ask the shared routing matrix for a non-local
provider model, while unpinned local-eligible agents default to
`omlx/coding-workhorse` when available. If no non-local matrix pick exists for a
local-forbidden agent, the spawn fails closed instead of inheriting a possibly
local session default. Explicit wrapper `model:` pins still win.
