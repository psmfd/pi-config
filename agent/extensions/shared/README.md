...
| `expertise-api-config.ts` | `buildClientConfig`, `isLoopbackHost`, `parseEnvFile`, `loadEnvLocal`, `ClientConfig`, `ConfigResult`, `DEFAULT_BASE_URL`, `ENV_*` | agent-expertise-api config resolution + trust-boundary validation (ADR-0028): `process.env` > `.env.local` > defaults, loopback-only base URL, API key required. Includes the pure `.env.local` parse/load helpers; path ANCHORING stays consumer-local (`expertise-client/lib/env.ts`, the fanout gate's sibling resolver). Moved here from `expertise-client/lib/config.ts` + `env.ts` (ADR-0095) so `expertise-fanout-gate` shares one parser — same #635/ADR-0088 rationale as `secret-scan.ts`. |
| `expertise-api-http.ts` | `apiGet`, `apiPost`, `errorDetail`, `ApiResponse`, `MAX_BODY_BYTES` | Bounded, credentialed HTTP for the loopback expertise API: `Authorization: Bearer` injected (never echoed), `redirect: "error"`, 256 KB body cap, injectable `fetchImpl`. Moved from `expertise-client/lib/http.ts` (ADR-0095). |
| `expertise-api-health.ts` | `checkReady` | Unauthenticated-shape `/health/ready` preflight against the configured loopback base. Moved from `expertise-client/lib/health.ts` (ADR-0095). |
