...
| `session-unavailable.ts` | `sessionUnavailableModels`, `isProviderRateLimited`, `markSessionUnavailable`, `clearSessionUnavailable` | ADR-0122 process-local dynamic provider deny evidence shared by auto-router and subagent runtime failover. Accepts qualified `provider/id` keys only; a conservative 429/quota classifier drives one pre-tool child retry. Separate from the immutable availability snapshot and reviewed capability policy; session start or explicit `--retry-unavailable` refresh clears it. |
