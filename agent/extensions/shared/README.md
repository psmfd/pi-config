...
| `session-unavailable.ts` | `createSessionDeny`, `sessionDeny`, `isProviderRateLimited`, `markSessionUnavailable`, `clearSessionUnavailable`, `providerOf`, `SessionDeny`, `ModelDenyView`, `DenyRecord` | ADR-0122/ADR-0126 process-local dynamic provider deny evidence shared by auto-router and subagent policy. **Two scopes:** `model` (one qualified `provider/id`) and `provider` (the ADR-0126 circuit breaker — every row of a provider, including ones never probed). One `has("provider/id")` view feeds `resolveCapabilityPick`/`resolveTierPick`/`buildRoutingPrompt`, so a plain `Set<string>` still satisfies the read side. Records carry `scope`/`source`/`reason`/`at` provenance and are first-writer-wins; a conservative 429/quota classifier drives one pre-tool child retry (ADR-0122) and, at two distinct rate-limited models of one provider, trips the breaker. `createSessionDeny()` yields isolated instances for tests; `sessionDeny` is the canonical process-local one. Separate from the immutable availability snapshot and reviewed capability policy; session start clears every scope, `--retry-unavailable` clears runtime evidence but preserves operator disables. |
