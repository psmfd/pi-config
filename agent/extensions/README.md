...
| [`bash-destructive-guard/`](bash-destructive-guard/README.md) | **First-party** to this repo | n/a | Denies `bash` tool calls invoking destructive operations (`rm`/`mv`, output-redirect clobber, `find -delete`, `dd of=`, `truncate`) against paths outside a configurable safe list; hardened against the GuardFall pipe-sink and glued-substitution bypasses (ADR-0072). Consumes [`shared/shell-lex.ts`](shared/README.md). Companion to `secrets-guard`. |
...
| [`compaction-optimizer/`](compaction-optimizer/README.md) | **First-party** to this repo | [`psmfd/pi-compaction-optimizer`](https://github.com/psmfd/pi-compaction-optimizer) | Dual-handler compaction (`session_before_compact` + `session_compact`): pre-compaction archive + file-tracker pruning + deterministic/hybrid/llm-only summary modes; establishes the `extensionSettings.<name>.*` namespace and per-extension data subtree (ADR-0019). |
| [`expertise-client/`](expertise-client/README.md) | **First-party** to this repo | [`psmfd/pi-expertise-client`](https://github.com/psmfd/pi-expertise-client) | Local-only client for the loopback `agent-expertise-api`: registers read-side `expertise_search` + create-only `expertise_create` (write opt-in, secret-scanned; ADR-0028/0029). Transport/config stack lives in [`shared/`](shared/README.md) `expertise-api-*` since ADR-0095. |
