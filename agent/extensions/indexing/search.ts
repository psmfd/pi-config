...
/**
 * Build the `ccc search` argv. The query is split into positional words (the
 * CLI declares `QUERY...` and re-joins them); spawning with an arg array means
 * no shell interpretation, so untrusted query text cannot inject.
 */
export function buildSearchArgs(query: string, options: SearchArgsOptions): string[] {
  const words = query.trim().split(/\s+/).filter((w) => w.length > 0);
  const limit = clampLimit(options.limit);
  const args = ["search", ...words, "--limit", String(limit)];
