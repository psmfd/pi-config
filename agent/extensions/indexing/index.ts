...
import { assertCliInvocation, defaultLauncher, defaultRunner, resolveBinary } from "./ccc.ts";
import { classifyOutput } from "./parse.ts";
import { PINNED_CCC_VERSION } from "./pin.ts";
import { Reindexer } from "./reindex.ts";
import { buildSearchArgs, clampLimit, formatResultsForModel, validatePathFilter } from "./search.ts";
...
export default function indexing(pi: ExtensionAPI): void {
  ...
  let reindexer = new Reindexer({ launcher: defaultLauncher, binary, cwd: projectRoot, env: process.env });
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    reindexer = new Reindexer({ launcher: defaultLauncher, binary, cwd: projectRoot, env: process.env });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      limit: Type.Optional(Type.Number({ description: "Max results (1-25; default from config)." })),
  ...
  pi.registerCommand("index", {
    ...
    handler: async (args, ctx) => {
      ...
      if (sub === "build") {
        try {
          assertCliInvocation(binary, ["index"]);
          const proc = defaultLauncher(binary, ["index"], { cwd: projectRoot, env: process.env });
          ctx.ui.notify(`indexing: re-index started in background (pid ${proc.pid ?? "?"})`, "info");
        } catch (err) {
  ...
  pi.on("agent_end", (_event, ctx) => {
    ...
    reindexer.maybeReindex(enabled, sessionIdle(ctx));
  });
}
