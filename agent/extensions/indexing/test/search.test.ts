...
test("buildSearchArgs splits the query into positional words", () => {
  const args = buildSearchArgs("idle gated reindex", { limit: 5 });
  assert.deepEqual(args, ["search", "idle", "gated", "reindex", "--limit", "5"]);
});

test("buildSearchArgs appends --lang and --path when given", () => {
  const args = buildSearchArgs("foo", { limit: 3, lang: "typescript", path: "agent/**" });
  assert.deepEqual(args, ["search", "foo", "--limit", "3", "--lang", "typescript", "--path", "agent/**"]);
});

test("buildSearchArgs collapses whitespace and ignores blank lang/path", () => {
  const args = buildSearchArgs("  a   b  ", { limit: 2, lang: "  ", path: "" });
  assert.deepEqual(args, ["search", "a", "b", "--limit", "2"]);
});
