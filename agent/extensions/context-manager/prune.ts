...
export function pruneToolResult(message: ToolResultMessage, maxResultBytes: number): ToolResultMessage {
  ...
  const marker = `\n\n[context-manager: elided ${elided} chars from ${message.toolName} — re-run the tool for full output]\n\n`;

  const nonText = message.content.filter((b) => b.type !== "text");
  return { ...message, content: [{ type: "text", text: head + marker + tail }, ...nonText] };
}
