...
> **Pinned to pi `v0.80.10-psmfd.1`** (source: PSMFD-attested rebuilds on
> [`psmfd/pi`](https://github.com/psmfd/pi/releases), upstream base `0.80.10`).
>
...
As of the v0.80.6-psmfd.1 bump, this runtime pin tracks pi `0.80.6`. The vendored subagent snapshot remains paired to pi `0.80.2` (re-paired 2026-07-07, #396); the gap was audited at the 0.80.6 bump — upstream's `examples/extensions/subagent/` is byte-identical between v0.80.2 and v0.80.6 (`git diff` empty over the range), so no re-pair is required. Patch #3 (`tool_execution_*` UI refresh) was **still required** at `0.80.2` — upstream's example still ships the dead `tool_result_end` branch while `agent-session` emits `tool_execution_{start,update,end}` per `pi/docs/rpc.md` § 855–898 — and was **expanded** in the re-pair to also consume `tool_execution_update` (closes #46). Patches #4a–d (Copilot fallback rung, oMLX liveness probe, spawn-time pin gate, supporting imports — our own ADR-0080/ADR-0081 policy layer) were previously undocumented in the subagent patch table and were formalised in the re-pair; the mechanical check to prevent recurrence is tracked as #582. Earlier patches #1/#2 were dropped at the 0.75.4 re-audit (#136) once upstream adopted them per [earendil-works/pi#4710](https://github.com/earendil-works/pi/issues/4710). Future bumps should continue to consider both pins together.
