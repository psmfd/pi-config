...
info "Running shellcheck over scripts/, hooks/, install.sh, update.sh, install-expertise.sh, setup.sh"
if ! command -v shellcheck >/dev/null 2>&1; then
  err "shellcheck: binary not on PATH (vendored pin agent/vendor/shellcheck/, ADR-0011); shell lint gate cannot run"
else
  sc_targets="$(find scripts hooks -name '*.sh' -type f; ls install.sh update.sh install-expertise.sh setup.sh 2>/dev/null)"
  sc_count="$(printf '%s\n' "$sc_targets" | grep -c .)"
