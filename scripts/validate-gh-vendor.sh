#!/usr/bin/env bash
#
# validate-gh-vendor.sh — structural validator for agent/vendor/gh/.
#
...
# Network-free; safe in CI without GitHub access.
#
...
cd "$REPO_DIR" || { echo "validate-gh-vendor.sh: cannot cd to $REPO_DIR" >&2; exit 2; }

VENDOR_DIR="agent/vendor/gh"
VERSION_FILE="$VENDOR_DIR/VERSION"
CHECKSUMS_FILE="$VENDOR_DIR/CHECKSUMS"
README_FILE="$VENDOR_DIR/README.md"

errors=0
err() { printf 'ERROR gh-vendor: %s\n' "$*" >&2; errors=$((errors + 1)); }
...
# Expected assets are tag-templated for gh (the tag appears in each filename).
ver_bare="${version_value#v}"
EXPECTED_ASSETS="gh_${ver_bare}_linux_amd64.tar.gz gh_${ver_bare}_linux_arm64.tar.gz gh_${ver_bare}_macOS_amd64.zip gh_${ver_bare}_macOS_arm64.zip"
...
if [ ! -f "$README_FILE" ]; then
  ...
  if [ -n "$ver_bare" ] && ! grep -qF "$ver_bare" "$README_FILE"; then
