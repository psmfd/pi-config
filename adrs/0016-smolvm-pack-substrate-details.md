...
- **Tracking issues:** [#99](https://github.com/TheSemicolon/pi_config/issues/99) (distribution umbrella), [#131](https://github.com/TheSemicolon/pi_config/issues/131) (ζ implementation parent), [#159](https://github.com/TheSemicolon/pi_config/issues/159) (this design pass), [#160](https://github.com/TheSemicolon/pi_config/issues/160) (Dockerfile + entrypoint shim — consumes this ADR), [#161](https://github.com/TheSemicolon/pi_config/issues/161) (CI release workflow), [#162](https://github.com/TheSemicolon/pi_config/issues/162) (cosign provenance), [#163](https://github.com/TheSemicolon/pi_config/issues/163) (UX docs), [#164](https://github.com/TheSemicolon/pi_config/issues/164) (smolvm vendor pin), [#119](https://github.com/TheSemicolon/pi_config/issues/119) (V1+V2 verification — closed PASS, supplies the design inputs that anchor this ADR)

## Context

[ADR-0013](0013-distribution-substrate-strategy.md) selected ζ as a substrate but explicitly deferred four implementation dimensions: persistence-model R1 mechanism, bake matrix shape (2 vs 4 packs), registry namespace, and update story. [ADR-0014](0014-oci-substrate-amendment-to-0013.md) subsequently reframed ζ as **the** sandbox substrate (load-bearing security capability — microVM-grade isolation) rather than a coverage convenience, and added two cross-substrate amendments that apply to ζ (`PI_OFFLINE=1` + `PI_SKIP_VERSION_CHECK=1` baked env; cosign-keyless as floor for OCI substrates, SHA256SUMS for non-OCI). Issue [#119](https://github.com/TheSemicolon/pi_config/issues/119) closed PASS on both V1 (node-less extension portability — all 5 of our extensions load under the vendored Bun-compiled pi binary in a `--network none` debian:bookworm-slim container) and V2 (`smolvm pack run -e KEY=VALUE` env-var plumbing — documented `-e KEY=VALUE` form works on smolvm 0.7.2; undocumented Docker-style `-e KEY` inherit-from-host shorthand does not).
...
The **`.smolmachine` sidecar** portion is NOT byte-reproducible at all: smolvm v0.7.2 embeds an RFC-3339 `created` timestamp in the manifest and does not honor `SOURCE_DATE_EPOCH`. See [#170](https://github.com/TheSemicolon/pi_config/issues/170) for the upstream-follow-up and the cosign policy implication (attestations bind to produced digest, not bit-identity). #162's policy design must accommodate this asymmetry between the two halves of D3.
